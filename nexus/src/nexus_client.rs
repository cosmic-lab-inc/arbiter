...
impl NexusClient {
  ...
  pub async fn historical_transactions(
    ...
    let sigs = Self::historical_signatures(rpc, key, limit).await?;
  ...
  pub async fn accounts(rpc: &RpcClient, keys: &[Pubkey]) -> anyhow::Result<Vec<AcctCtx>> {
